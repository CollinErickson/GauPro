...
To simplify, we can multiply it by -1/2, and call this the deviance, denoted here as $\mathcal{D}$.

$$ \mathcal{D} = \log L = k \log(2 \pi) +  \log|\Sigma|  +  (Y - \mu) \Sigma^{-1} (Y - \mu)  $$
$k$ can be ignored since it usually constant while optimizing parameters.
$$ \mathcal{D} = \log L \propto  \log|\Sigma|  +  (Y - \mu) \Sigma^{-1} (Y - \mu)  $$
