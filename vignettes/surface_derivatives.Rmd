...
$$ Cov(\frac{\partial y(x)}{\partial x_i},\frac{\partial y(x)}{\partial x_j})$$
We can write this as a limit.

$$ \lim_{\delta \rightarrow 0} Cov\left(\frac{y(x+\delta e_i) - y(x)}{\delta}, \frac{y(x+\delta e_j) - y(x)}{\delta}\right)$$
The covariance function is bilinear, so we can split it into four terms.

$$ \lim_{\delta \rightarrow 0} \frac{1}{\delta^2} \left(   Cov\left[y(x+\delta e_i), y(x+\delta e_j)\right] - Cov\left[y(x+\delta e_i), y(x)\right] - Cov\left[y(x), y(x+\delta e_j)\right] + Cov\left[y(x), y(x)\right] \right)$$
This can be recognized as the second order derivative of $Cov(y(u), y(v))$ with respect to $u_i$ and $v_j$ evaluated at $u=v=x$. See finite difference differentiation for more details. We have to use $u$ and $v$ instead of a single $x$ to be clear which component of the covariance function is being differentiated.

Thus we have the following. It looks obvious, I'm not sure I even needed the previous step.
$$ Cov(\frac{\partial y(x)}{\partial x_i},\frac{\partial y(x)}{\partial x_j}) = \frac{\partial Cov(y(u), y(v))}{\partial u \partial v} |_{u=x, v=x}$$
Let $U$ be the matrix with rows $u$ and $v$.
Recall that
$$ Cov(U) = \Sigma_{U} - \Sigma_{UX} \Sigma_{X}^{-1} \Sigma_{XU}$$
The $(1,2)$ element of this is the covariance we are looking for.
$$ Cov(y(u), y(v)) = \Sigma_{u,v} - \Sigma_{u,X} \Sigma_X^{-1} \Sigma_{X,v}$$
Now we need to differentiate this with respect to $u_i$ and $v_j$.

$$ Cov(\frac{\partial y(x)}{\partial x_i},\frac{\partial y(x)}{\partial x_j}) =   \frac{\partial^2 \Sigma_{u,v}}{\partial u_i \partial v_j} - \frac{\partial \Sigma_{u,X}}{\partial u_i} \Sigma_X^{-1} \frac{\partial \Sigma_{X,v}}{\partial v_j} |_{u=x,v=x}$$
Therefore we have found the distribution of the gradient.
