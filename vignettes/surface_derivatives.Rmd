...
$$\frac{\partial^2 y(x)}{\partial x_i \partial x_k} = \frac{\partial^2 \mu(x)}{\partial x_i \partial x_k} +\frac{\partial^2 \Sigma_{xX}}{\partial x_i \partial x_k} \Sigma_X^{-1}(Y_X - \mu_X)$$
...
$$R(x, u_j) = \exp \left(-\sum_{\ell =1}^d \theta_\ell (x_\ell - u_{j\ell})^2 \right) $$

$$\frac{\partial R(x, u_j)}{\partial x_i}  = -2\theta_i (x_i - u_{ji}) \exp \left(-\sum_{\ell =1}^d \theta_\ell (x_\ell - u_{j\ell})^2 \right)  $$
The second derivative with respect to the same dimension is

$$\frac{\partial^2 R(x, u_j)}{\partial x_i^2}  = \left(-2\theta_i  + 4\theta_i^2 (x_i - u_{ji})^2 \right) \exp \left(-\sum_{\ell =1}^d \theta_\ell (x_\ell - u_{j\ell})^2 \right)  $$

The cross derivative is 
$$\frac{\partial^2 R(x, u_j)}{\partial x_i \partial x_k}  = 4\theta_i \theta_k (x_i - u_{ji}) (x_k - u_{jk}) \exp \left(-\sum_{\ell =1}^d \theta_\ell (x_\ell - u_{j\ell})^2 \right)  $$
...
$$ \frac{\partial y(x)}{\partial x} = \lim_{\delta \rightarrow 0} \frac{1}{\delta} \left( \begin{bmatrix} y(x+\delta e_1) \\ \vdots \\ y(x+\delta e_d) \end{bmatrix} - y(x) \right)$$
...
$$ E \left[\frac{\partial y(x)}{\partial x} \right] =  \frac{\partial E[y(x)]}{\partial x}  \\
   = \frac{\partial \Sigma(x,X)}{\partial x_i}^T \Sigma_X^{-1}(Y_X - \mu_X)
$$
...
$$ \lim_{\delta \rightarrow 0} \frac{1}{\delta^2} \left(   \text{Cov}\left[y(x+\delta e_i), y(x+\delta e_j)\right] - \text{Cov}\left[y(x+\delta e_i), y(x)\right] - \text{Cov}\left[y(x), y(x+\delta e_j)\right] + \text{Cov}\left[y(x), y(x)\right] \right)$$
...
Thus we have the following. It looks obvious, I'm not sure I even needed the previous step.
...
Recall that
$$ \text{Cov}(U) = \Sigma_{U} - \Sigma_{UX} \Sigma_{X}^{-1} \Sigma_{XU}$$
...
$$
\begin{align}
  \text{Var}[Q(X)] &= \sum_{j=1}^n \lambda_j \text{Var} \left[ (U_j + b_j)^2 \right] \\
