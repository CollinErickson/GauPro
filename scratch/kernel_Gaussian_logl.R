...
#' @examples
#' k1 <- Gaussian_logl$new(logl=0)
Gaussian_logl <- R6::R6Class(classname = "GauPro_kernel_Gaussian_logl",
  ...
  public = list(
    ...
    logs2_lower = NULL,
    logs2_upper = NULL,
    initialize = function(logl, s2=1, logl_lower=-8, logl_upper=6,
                          s2_lower=1e-8, s2_upper=1e8) {
      self$logl <- logl
      self$logl_length <- length(logl)
      # if (length(theta) == 1) {
      #   self$theta <- rep(theta, self$d)
      # }
      self$logl_lower <- logl_lower
      self$logl_upper <- logl_upper
    ...
    k = function(x, y=NULL, logl=self$logl, s2=self$s2, params=NULL) {
      if (!is.null(params)) {
    ...
    param_optim_start = function(jitter=F, y) {
      ...
      vec <- c(self$logl, self$logs2)
      if (jitter) {
        # vec <- vec + c(self$beta_optim_jitter,  0)
        vec[1:length(self$logl)] = vec[1:length(self$logl)] + rnorm(length(self$logl), 0, 1)
      }
      vec
    },
    param_optim_start0 = function(jitter=F, y) {
      ...
      vec <- c(rep(0, self$logl_length), 0)
      if (jitter) {
        vec[1:length(self$logl)] = vec[1:length(self$logl)] + rnorm(length(self$logl), 0, 1)
      }
      vec
    ...
    dC_dparams = function(params=NULL, C, X, C_nonug, nug) {#browser(text = "Make sure all in one list")
      ...
      dC_dlogls <- rep(list(C_nonug), length(logl))
      n <- nrow(X)
      for (k in 1:length(logl)) {
        for (i in seq(1, n-1, 1)) {
          for (j in seq(i+1, n, 1)) {
            dC_dlogls[[k]][i,j] <- dC_dlogls[[k]][i,j] * (X[i,k] - X[j,k])^2 / l[k]^2 * log10
            dC_dlogls[[k]][j,i] <- dC_dlogls[[k]][i,j]
          }
        }
      ...
      mats <- c(dC_dlogls, list(dC_dlogs2))
