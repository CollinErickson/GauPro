...
# Check dC_dtheta
m1 <- (gp$kernel$k(gp$X, l=1+1e-6) - gp$kernel$k(gp$X, l=1-1e-6)) / 1e-6 / 2
C_nonug <- gp$kernel$k(gp$X, l=1)
C <- C_nonug + diag(gp$nug, nrow(C_nonug))
m2 <- gp$kernel$dC_dparams(params = c(0, 1), X = gp$X, C = C, C_nonug = C)[[1]][[1]]
c(m1 * log(10) -m2) %>% summary
