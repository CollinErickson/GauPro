...
RatQuad <- R6::R6Class(classname = "GauPro_kernel_RatQuad",
  ...
  public = list(
    ...
    dC_dparams = function(params=NULL, X, C_nonug, C, nug) {#browser(text = "Make sure all in one list")
      n <- nrow(X)
      if (is.null(params)) {params <- c(self$beta, self$alpha, self$logs2)}
      ...
      for (i in seq(1, n-1, 1)) {
        for (j in seq(i+1, n, 1)) {
          r2 <- sum(theta * (X[i,]-X[j,])^2)
          t1 <- 1 + r2 / alpha
          dC_dparams[lenparams-1, i,j] <- C[i,j] * (- log(t1) + r2 / alpha / t1)
