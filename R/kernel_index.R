...
IndexKernel <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(s2=1, D,
      ...
      self$p_upper <-rep(1, self$p_length)
      # self$logp <- log(p, 10)
    ...
    k = function(x, y=NULL, p=self$p, logalpha=self$logalpha, s2=self$s2, params=NULL) {#browser()
      if (!is.null(params)) {
    ...
    kone = function(x, y, p, s2) {
      # if (missing(p)) {p <- 10^logp}
      # out <- s2 * exp(-sum(alpha*sin(p * (x-y))^2))
      stopifnot(x>=1, y>=1, x<=self$D, y<=self$D)
      if (x==y) {
        out <- s2 * 1
      } else {
        # i <- x-1
