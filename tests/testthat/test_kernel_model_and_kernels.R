...
test_that("check factor kernels", {
  n <- 20
  d <- 2
  x <- matrix(runif(n*d), ncol=d)
  ...
  kern_list <- list(
    FactorKernel$new(D=2, nlevels=3, xindex=2),
    OrderedFactorKernel$new(D=2, nlevels=3, xindex=2),
    LatentFactorKernel$new(D=2, nlevels=3, xindex=2, latentdim = 2)
  )
  for (j in 1:length(kern_chars)) {
    kern_char <- kern_chars[j]
    cat(j, kern_char, "\n")
